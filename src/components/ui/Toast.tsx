...
const STYLES: Record<ToastType, { box: string; icon: string; name: IconName }> = {
  success: { box: "bg-success-surface border-success/25", icon: "text-success", name: "success" },
  error: { box: "bg-error-surface border-error/25", icon: "text-error", name: "error" },
  warning: { box: "bg-warning-surface border-warning/30", icon: "text-warning", name: "warning" },
  info: { box: "bg-info-surface border-info/30", icon: "text-info", name: "info" },
};
...
export function Toast({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-base font-bold text-heading">{toast.message}</span>
    </button>
  );
}
